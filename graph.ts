...
export class Graph {
  ...
  setGraphFormat(f: GraphFormat) {
    ...
    this.reset()
    //TODO: everything must include modules as well, sensors are double
    this.numNodes = f.sensors.length * 2 + f.moduleIds.length

    for(let i = 0; i < this.numNodes; i++){
      // if [i][j] == 1, i is the "parent" of j
    ...
    for(let i = 0; i < this.numNodes; i++){
      //this.sensorIDToIndex.set(f.sensors[i], i)
    }

    this.nodeHeight.fill(-1)
  ...
  getSensorHeight(sensorIndex: number){
    if(this.sensorHeight[sensorIndex] != -1){
      return this.sensorHeight[sensorIndex]
    } 
  }

  //returns indicies of the sensor's children
  findSensorChildren(sensorIndex: number){
    let result = new Array
    for(let i = 0; i < this.numSensors; i++){
      if(this.adjacencyMatrix[sensorIndex][i] == 1){
        result.push(i)
      }
    }
    return result
